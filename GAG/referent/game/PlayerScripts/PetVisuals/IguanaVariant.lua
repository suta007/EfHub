...
local function v_u_17(p11)
	...
	if v_u_2:HasTag(p11, "IguanaVariantColored") then
		return
	end
	if p11.Name == "Iguana" and (p11.Parent and p11.Parent.Parent) then
		...
		if v12:IsA("Tool") then
			...
			if typeof(v16) == "string" then
				...
				if v16 then
					if #v_u_3 == 0 then
						v_u_3 = require(v_u_1.Modules.SelectiveAssetReplication):GetAssetAsync("Skins", "Iguana Variants"):GetChildren()
					end
					if #v_u_3 == 0 then
						warn("No Iguana variants found in folder")
					else
						local v13 = v_u_3
						local v14 = 0
						for v15 = 1, #v16 do
						...
						v_u_2:AddTag(p11, "IguanaVariantColored")
					end
				else
					return
				end
...
for _, v18 in v_u_2:GetTagged("IguanaVariant") do
	if typeof(v18) == "Instance" and v18:IsA("Model") then
		v_u_17(v18)
	end
end
v_u_2:GetInstanceAddedSignal("IguanaVariant"):Connect(function(p19)
