...
local function v_u_14(p11)
	if p11.Name == "Swan" and (p11.Parent and p11.Parent.Parent) then
...
local function v_u_20(p15)
	...
	if v_u_2:HasTag(p15, "SwanVariantColored") then
		return
	else
		...
		if v16 then
			...
			local v19 = v17 % 100000 == 0
			v_u_3 = v_u_3 or require(v_u_1.Modules.SelectiveAssetReplication):GetAssetAsync("Skins", "Swan Variants"):GetChildren()[1]
			if not v_u_3 then
				warn("Swan: No special variant found in \'Swan Variants\'")
			end
			if v19 and v_u_3 then
				v_u_10(p15, v_u_3)
			end
			v_u_2:AddTag(p15, "SwanVariantColored")
		end
	end
end
for _, v21 in v_u_2:GetTagged("SwanVariant") do
	if typeof(v21) == "Instance" and v21:IsA("Model") then
		v_u_20(v21)
	end
end
v_u_2:GetInstanceAddedSignal("SwanVariant"):Connect(function(p22)
